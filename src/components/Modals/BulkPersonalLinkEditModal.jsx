...
const BulkPersonalLinkEditModal = ({ show, id, initialData = null, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ Add guest_email only if not registered
      if (!isRegistered && formData.guest_email) {
        ...
        <div className="p-5">
          {loading ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-sm">
              ...
              {/* Email - editable if not registered */}
<div>
...
//           expire_date: d.expire_date || "",
//           guest_email: d.guest_email || "",
//           registered: Boolean(d.registered === true || d.registered === "true"),
//           source_type: d.source_type || "",
//         };
//         setFormData(normalized);

//         // Fetch ticket list only if not registered
//         if (!normalized.registered) {
...
//   const isRegistered = formData.registered;
//   const today = new Date().toISOString().split("T")[0];
...
//               {isRegistered && (
//                 <div className="mb-4 bg-yellow-50 border border-yellow-300 text-yellow-700 px-3 py-2 rounded text-sm">
//                   This guest is already registered. Only name and company can be
//                   edited.
//                 </div>
//               )}
